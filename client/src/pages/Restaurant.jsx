...
export default function Restaurant({ user }) {
  ...
  const cycleStatus = async item => {
    if (!canManage) return;
    const cycle = ["available", "unavailable", "ongoing"];
          ...
          <>
            <div style={{ display: "flex", gap: "8px", marginBottom: "18px", alignItems: "center", flexWrap: "wrap" }}>
              {[
                ...
                { val: "all",    label: "All Orders",          count: orders.length, accent: "#555"    },
              ].map(f => (
                <button key={f.val} onClick={() => setOrderFilter(f.val)}
                  style={{
                    padding: "8px 16px",
                    border: `1.5px solid ${orderFilter === f.val ? f.accent : "#ccdacc"}`,
                    borderRadius: "10px",
                    background: orderFilter === f.val
                      ? f.val === "queued" ? "#fffbeb" : f.val === "active" ? "#ecfdf5" : "#f4f6f0"
                      : "#fff",
            ...
            {orderFilter === "queued" && queuedCount > 0 && (
              <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: "12px", padding: "14px 18px", marginBottom: "16px", display: "flex", alignItems: "flex-start", gap: "10px", fontSize: ".84rem", color: "#b45309" }}>
                ...
                <div>
                  ...
                  They move to <strong>Active Orders</strong> automatically when the guest checks in.
                </div>
              </div>
            )}
      ...
      {showModal && (
        <div className="mo" onClick={() => setShowModal(false)}>
          <div className="mb" onClick={e => e.stopPropagation()}>
            <div className="mh">
              <div>
                <p className="mh-title">{editItem ? "Edit Menu Item" : "Add Menu Item"}</p>
                <p className="mh-sub">{editItem ? "Update item details" : "Add a new item to the menu"}</p>
              </div>
        ...
        <div className="mo" onClick={() => setDelItem(null)}>
          <div className="mb del-modal" onClick={e => e.stopPropagation()}>
            <div className="mh" style={{ background: "linear-gradient(135deg,#c62828,#e53935)" }}>
              <div>
                <p className="mh-title">Delete Item</p>
                <p className="mh-sub">This action cannot be undone</p>
              </div>
