...
import {
  RiHotelBedLine, RiCheckboxCircleLine, RiErrorWarningLine, RiToolsLine,
  RiPencilLine, RiDeleteBinLine, RiVipCrownLine, RiStarLine,
} from "react-icons/ri";
...
const TYPE_ICON = {
  Standard: RiHotelBedLine,
  Deluxe:   RiStarLine,
  Suite:    RiVipCrownLine,
};
...
.btn-primary { padding:11px 22px; background:#07713c; color:#fff; border:none; border-radius:10px; cursor:pointer; font-size:.88rem; font-weight:700; font-family:Arial,sans-serif; box-shadow:0 4px 14px rgba(7,113,60,.28); white-space:nowrap; display:inline-flex; align-items:center; gap:6px; }
...
export default function Rooms() {
  ...
  const [form, setForm] = useState({ room_number: "", type: "Standard", floor: "", price: "", status: "available", description: "" });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = { room_number: form.room_number, type: form.type, floor: parseInt(form.floor), price: parseFloat(form.price), status: form.status, description: form.description };
    if (editRoom) {
  ...
  const filtered = rooms.filter(r => {
    const ms = r.room_number.toLowerCase().includes(search.toLowerCase()) || r.type.toLowerCase().includes(search.toLowerCase());
    return ms && (filterStatus === "all" || r.status === filterStatus) && (filterType === "all" || r.type === filterType);
  });

  const STAT_CARDS = [
    { lbl: "Total Rooms",  val: rooms.length,                                        Icon: RiHotelBedLine,        bg: "#e8f5e9", c: "#1b5e20" },
    { lbl: "Available",    val: rooms.filter(r => r.status === "available").length,  Icon: RiCheckboxCircleLine,  bg: "#e3f2fd", c: "#1565c0" },
    { lbl: "Occupied",     val: rooms.filter(r => r.status === "occupied").length,   Icon: RiErrorWarningLine,    bg: "#fce4ec", c: "#c62828" },
    { lbl: "Maintenance",  val: rooms.filter(r => r.status === "maintenance").length,Icon: RiToolsLine,           bg: "#fff3e0", c: "#e65100" },
  ];
      ...
      <div className="page">
        <div className="page-hdr">
          ...
          <button className="btn-primary" onClick={openAdd}>
            <span style={{ fontSize: "1rem", lineHeight: 1 }}>+</span> Add Room
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="room-grid">
            {filtered.map(r => {
              ...
              const Icon = TYPE_ICON[r.type] || RiHotelBedLine;
              return (
                <div key={r.id} className="room-card">
                  ...
                  <div className="room-body">
                    ...
                    {r.description && <div style={{ fontSize: ".82rem", color: "#9aaa9a", marginBottom: "12px", lineHeight: "1.5" }}>{r.description}</div>}
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button className="ba ba-edit" style={{ flex: 1, justifyContent: "center" }} onClick={() => openEdit(r)}>
                        <RiPencilLine size={14} /> Edit
                      </button>
                      <button className="ba ba-del" style={{ flex: 1, justifyContent: "center" }} onClick={() => handleDelete(r.id)}>
          ...
          <div className="mb" style={{ maxWidth: "460px" }} onClick={e => e.stopPropagation()}>
            ...
            <div className="mbody">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "14px" }}>
                <div><label className="flabel">Room Number</label><input className="fi" value={form.room_number} onChange={e => setForm({ ...form, room_number: e.target.value })} placeholder="e.g. 101" /></div>
                <div><label className="flabel">Floor</label><input type="number" className="fi" value={form.floor} onChange={e => setForm({ ...form, floor: e.target.value })} placeholder="e.g. 1" /></div>
                <div style={{ gridColumn: "1/-1" }}><label className="flabel">Price / Night (₱)</label><input type="number" className="fi" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} placeholder="e.g. 1500" /></div>
                <div><label className="flabel">Room Type</label><select className="fi" value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={{ cursor: "pointer" }}>{["Standard","Deluxe","Suite"].map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                <div><label className="flabel">Status</label><select className="fi" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} style={{ cursor: "pointer" }}>{Object.entries(ROOM_STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
              </div>
              <div><label className="flabel">Description (Optional)</label><textarea className="fi" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="e.g. Ocean view room..." rows={3} style={{ resize: "vertical" }} /></div>
            </div>
            <div className="mfoot">
              <button className="btn-cancel" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="btn-confirm" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : editRoom ? "Save Changes" : "Add Room"}</button>
            </div>
          </div>
        </div>
      )}
