...
import {
  ...
  RiPencilLine, RiDeleteBinLine, RiAddLine,
} from "react-icons/ri";
...
export default function Reservations() {
  ...
  const [form, setForm] = useState({ guest_name: "", guest_email: "", guest_phone: "", room_id: "", check_in: "", check_out: "", status: "confirmed", notes: "" });
  ...
  const openAdd = () => { setEditRes(null); setForm({ guest_name: "", guest_email: "", guest_phone: "", room_id: "", check_in: "", check_out: "", status: "confirmed", notes: "" }); setError(""); setSuccess(""); setShowModal(true); };
  const openEdit = (res) => { setEditRes(res); setForm({ guest_name: res.guest_name, guest_email: res.guest_email || "", guest_phone: res.guest_phone || "", room_id: res.room_id || "", check_in: res.check_in, check_out: res.check_out, status: res.status, notes: res.notes || "" }); setError(""); setSuccess(""); setShowModal(true); };
  ...
  const handleSave = async () => {
    ...
    const payload = { guest_name: form.guest_name, guest_email: form.guest_email, guest_phone: form.guest_phone, room_id: form.room_id, room_number: rm?.room_number, check_in: form.check_in, check_out: form.check_out, status: form.status, total_amount: calcTotal(), notes: form.notes };
    if (editRes) { const { error: e } = await supabase.from("reservations").update(payload).eq("id", editRes.id); if (e) { setError(e.message); setSaving(false); return; } }
    else         { const { error: e } = await supabase.from("reservations").insert(payload);                      if (e) { setError(e.message); setSaving(false); return; } }
    ...
    if (editRes && editRes.room_id && editRes.room_id !== form.room_id) await supabase.from("rooms").update({ status: "available" }).eq("id", editRes.room_id);
    setSuccess(editRes ? "Reservation updated!" : "Reservation created!"); setSaving(false); fetchAll();
  ...
  const filtered = reservations.filter(r => {
    const ms = r.guest_name.toLowerCase().includes(search.toLowerCase()) || (r.room_number || "").includes(search) || (r.guest_email || "").toLowerCase().includes(search.toLowerCase());
    return ms && (filterStatus === "all" || r.status === filterStatus);
  });

  const STAT_CARDS = [
    { lbl: "Total",      val: reservations.length,                                      Icon: RiCalendarLine,       bg: "#e8f5e9", c: "#1b5e20" },
    { lbl: "Confirmed",  val: reservations.filter(r => r.status === "confirmed").length, Icon: RiCheckboxCircleLine, bg: "#e3f2fd", c: "#1565c0" },
    { lbl: "Checked In", val: reservations.filter(r => r.status === "checked_in").length,Icon: RiHome4Line,          bg: "#f3e5f5", c: "#6a1b9a" },
    { lbl: "Cancelled",  val: reservations.filter(r => r.status === "cancelled").length, Icon: RiCloseCircleLine,    bg: "#fce4ec", c: "#c62828" },
  ];
    ...
    <>
      ...
      <div className="page">
        <div className="page-hdr">
          <div><h2 className="page-title">Reservations</h2><p className="page-sub">Manage all guest reservations</p></div>
          <button className="btn-primary" onClick={openAdd}><RiAddLine size={16} /> New Reservation</button>
        </div>
        ...
        <div className="tc">
          <div className="tc-hdr"><div className="tc-title">All Reservations</div><span className="tc-badge">{filtered.length} results</span></div>
          <div className="tc-head" style={{ gridTemplateColumns: cols }}>
            {["Guest","Room","Check-In","Check-Out","Nights","Total","Status","Actions"].map(h => <div key={h} className="th">{h}</div>)}
          </div>
          <div className="tc-scroll">
            {loading ? <div className="empty">Loading...</div>
              : filtered.length === 0 ? <div className="empty">No reservations found.</div>
              : filtered.map(res => {
                ...
                return (
                  <div key={res.id} className="tr" style={{ gridTemplateColumns: cols }}>
                    <div className="rg">
                      ...
                      <div style={{ minWidth: 0 }}><div className="rg-name">{res.guest_name}</div>{res.guest_email && <div className="rg-sub">{res.guest_email}</div>}</div>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button className="ba ba-edit" onClick={() => openEdit(res)}><RiPencilLine size={13} /></button>
                      <button className="ba ba-del"  onClick={() => handleDelete(res)}><RiDeleteBinLine size={13} /></button>
                    </div>
                  </div>
                );
              })
      ...
      {showModal && (
        <div className="mo" onClick={() => setShowModal(false)}>
          <div className="mb" style={{ maxWidth: "600px" }} onClick={e => e.stopPropagation()}>
            <div className="mh">
              <div><p className="mh-title">{editRes ? "Edit Reservation" : "New Reservation"}</p><p className="mh-sub">{editRes ? "Update reservation details" : "Fill in the details below"}</p></div>
            ...
            <div className="mbody">
              ...
              <div className="sc2">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                  <div style={{ gridColumn: "1/-1" }}><label className="flabel">Full Name *</label><input className="fi" value={form.guest_name} onChange={e => setForm({ ...form, guest_name: e.target.value })} placeholder="e.g. Juan Dela Cruz" /></div>
                  <div><label className="flabel">Email</label><input type="email" className="fi" value={form.guest_email} onChange={e => setForm({ ...form, guest_email: e.target.value })} placeholder="guest@email.com" /></div>
                  <div><label className="flabel">Phone</label><input className="fi" value={form.guest_phone} onChange={e => setForm({ ...form, guest_phone: e.target.value })} placeholder="+63 9XX XXX XXXX" /></div>
                </div>
              </div>
              <div className="sc2">
                <div className="sc2-title">Room & Dates</div>
                ...
                {selectedRoom && (
                  <div style={{ background: "#e8f5e9", border: "1px solid #a5d6a7", borderRadius: "10px", padding: "12px 16px", marginBottom: "14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div><span style={{ fontWeight: "700", color: "#07713c" }}>Room {selectedRoom.room_number}</span><span style={{ color: "#6b7a6b", fontSize: ".85rem", marginLeft: "10px" }}>{selectedRoom.type} · Floor {selectedRoom.floor}</span></div>
                    <span style={{ fontWeight: "700", color: "#07713c" }}>₱{parseFloat(selectedRoom.price).toLocaleString()}/night</span>
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                  <div><label className="flabel">Check-In *</label><input type="date" className="fi" value={form.check_in} onChange={e => setForm({ ...form, check_in: e.target.value })} /></div>
                  <div><label className="flabel">Check-Out *</label><input type="date" className="fi" value={form.check_out} onChange={e => setForm({ ...form, check_out: e.target.value })} /></div>
                </div>
                {calcNights() > 0 && (
                  <div className="tot-banner">
                    <span style={{ color: "rgba(255,255,255,.72)", fontSize: ".86rem" }}>{calcNights()} nights × ₱{selectedRoom ? parseFloat(selectedRoom.price).toLocaleString() : 0}</span>
              ...
              <div className="sc2">
                ...
                <div style={{ marginBottom: "14px" }}><label className="flabel">Status</label><select className="fi" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} style={{ cursor: "pointer" }}>{Object.entries(STATUS_CFG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
                <div><label className="flabel">Notes (Optional)</label><textarea className="fi" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Any special requests..." rows={3} style={{ resize: "vertical" }} /></div>
              </div>
            </div>
            <div className="mfoot">
              <button className="btn-cancel" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="btn-confirm" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : editRes ? "Save Changes" : "Create Reservation"}</button>
            </div>
          </div>
        </div>
      )}
