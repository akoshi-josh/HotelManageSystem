...
import {
  RiCalendarLine, RiTimeLine, RiCalendarTodoLine, RiHotelBedLine,
  RiLoginBoxLine, RiWalkLine, RiCoinLine, RiBankCard2Line, RiSmartphoneLine, RiBankLine,
  RiAddLine,
} from "react-icons/ri";
...
.btn-primary { padding:11px 22px; background:#07713c; color:#fff; border:none; border-radius:10px; cursor:pointer; font-size:.88rem; font-weight:700; font-family:Arial,sans-serif; box-shadow:0 4px 14px rgba(7,113,60,.28); display:inline-flex; align-items:center; gap:7px; }
...
.ba { display:inline-flex; align-items:center; gap:5px; padding:5px 11px; border-radius:7px; border:1.5px solid; font-size:.74rem; font-weight:700; font-family:Arial,sans-serif; cursor:pointer; transition:all .15s; }
...
.btn-confirm { flex:2; padding:12px; background:#07713c; border:none; border-radius:10px; cursor:pointer; font-size:.9rem; font-weight:700; color:#fff; font-family:Arial,sans-serif; box-shadow:0 4px 14px rgba(7,113,60,.28); display:inline-flex; align-items:center; justify-content:center; gap:7px; }
...
export default function CheckIn() {
  ...
  const openCheckIn = (res) => {
    setSelected(res); setAmountPaid(res.total_amount?.toString() || "");
    setPayMethod("cash"); setSuccessMsg(""); setShowModal(true);
  };
  ...
  const calcWalkTotal = () => {
    ...
    return Math.max(0, (new Date(walkIn.check_out) - new Date(walkIn.check_in)) / 86400000) * parseFloat(rm.price);
  };
  ...
  const handleWalkIn = async () => {
    ...
    const { error } = await supabase.from("reservations").insert({ guest_name: walkIn.guest_name, guest_email: walkIn.guest_email, guest_phone: walkIn.guest_phone, room_id: walkIn.room_id, room_number: rm?.room_number, check_in: walkIn.check_in, check_out: walkIn.check_out, status: "checked_in", total_amount: total, notes: walkIn.notes, amount_paid: total });
  ...
  const filtered  = reservations.filter(r => r.guest_name.toLowerCase().includes(search.toLowerCase()) || (r.room_number||"").includes(search) || (r.guest_phone||"").includes(search));
  ...
  const STAT_CARDS = [
    { lbl: "Today's Arrivals", val: reservations.filter(r => r.check_in === today).length, Icon: RiCalendarLine,     bg: "#e8f5e9", c: "#1b5e20" },
    { lbl: "Overdue",          val: reservations.filter(r => r.check_in < today).length,   Icon: RiTimeLine,         bg: "#fff3e0", c: "#e65100" },
    { lbl: "Upcoming",         val: reservations.filter(r => r.check_in > today).length,   Icon: RiCalendarTodoLine, bg: "#e3f2fd", c: "#1565c0" },
    { lbl: "Available Rooms",  val: availRooms.length,                                      Icon: RiHotelBedLine,     bg: "#f3e5f5", c: "#6a1b9a" },
  ];
            ...
            {data.map(res => {
              ...
              return (
                <div key={res.id} className="tr" style={{ gridTemplateColumns: "2.4fr .8fr 1fr 1fr 1fr 1fr 1fr" }}>
                  <div className="rg"><div className="av">{res.guest_name.slice(0,2).toUpperCase()}</div><div><div className="rg-name">{res.guest_name}</div>{res.guest_phone && <div className="rg-sub">{res.guest_phone}</div>}</div></div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <button className="ba ba-in" onClick={() => openCheckIn(res)}><RiLoginBoxLine size={13} /> Check In</button>
                  </div>
                </div>
              );
            })}
    ...
    <>
      ...
      <div className="page">
        <div className="page-hdr">
          ...
          <button className="btn-primary" onClick={() => setShowWalkIn(true)}><RiWalkLine size={16} /> Walk-In Guest</button>
        </div>
        ...
        {loading ? <div className="empty">Loading...</div>
          : filtered.length === 0
            ? <div style={{ background: "#fff", borderRadius: "14px", padding: "60px", textAlign: "center", border: "1px solid #e4ebe4" }}>
                <RiCalendarLine size={48} color="#d1e8d1" style={{ marginBottom: "12px" }} />
        ...
        <div className="mo" onClick={() => setShowModal(false)}>
          <div className="mb" style={{ maxWidth: "500px" }} onClick={e => e.stopPropagation()}>
            <div className="mh" style={{ background: "linear-gradient(135deg,#07713c,#0a9150)" }}>
              <div><p className="mh-title">Process Check-In</p><p className="mh-sub">Confirm arrival and collect payment</p></div>
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <div className="pmg">
                    {PAYMENT_METHODS.map(({ key, label, Icon }) => (
                      <button key={key} className={`pm${payMethod===key?" on":""}`} type="button" onClick={() => setPayMethod(key)}>
                        <Icon size={16} />{label}
                      </button>
                    ))}
                  </div>
            ...
            <div className="mfoot">
              ...
              <button className="btn-confirm" onClick={handleCheckIn} disabled={processing||!amountPaid}>
                <RiLoginBoxLine size={16} />{processing?"Processing…":"Confirm Check-In"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="mo" onClick={() => setShowWalkIn(false)}>
          <div className="mb" style={{ maxWidth: "560px" }} onClick={e => e.stopPropagation()}>
            <div className="mh" style={{ background: "linear-gradient(135deg,#1565c0,#1976d2)" }}>
              <div><p className="mh-title">Walk-In Guest</p><p className="mh-sub">Guest arrives without prior reservation</p></div>
            ...
            <div className="mbody">
              ...
              <div className="sc2">
                <div className="sc2-title" style={{ color: "#1565c0" }}>Room & Dates</div>
            ...
            <div className="mfoot">
              ...
              <button className="btn-confirm" style={{ background: savingWalk?"#a8b8a8":"#1565c0", boxShadow:"0 4px 14px rgba(21,101,192,.3)" }} onClick={handleWalkIn} disabled={savingWalk}>
                <RiWalkLine size={16} />{savingWalk?"Checking In…":"Check In Now"}
              </button>
            </div>
          </div>
        </div>
